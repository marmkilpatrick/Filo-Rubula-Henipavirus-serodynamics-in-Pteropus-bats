...
NiV= function(Bjj,Bja,Baj,Baa,SA_A,LAA,LAJ){ #Model which simulated dynamics given parameter estimates
  ...
  for (t in st:tend) {#loop over weeks of data
    ...
    Sj[t+1]=max(Sj[t]+dSj,0);Ij[t+1]=max(Ij[t]+dIj,0);Rj[t+1]=max(Rj[t]+dRj,0);Mj[t+1]=max(Mj[t]+dMj,0)
    Sa[t+1]=max(Sa[t]+dSa,0);Ia[t+1]=max(Ia[t]+dIa,0);Ra[t+1]=max(Ra[t]+dRa,0)
...
fp2 <- mle2(ff2,start=list(#FiV starting/ending values
  Bjj=-3.85,Bja=-5.62,Baj=-5.58,Baa=-5.14,SA_A=1.41,
  LAA=-4.44,LAJ=-4.59));fp2 #fit model -1134.57

-1156.7 - (-1134.57) #Delta AIC FDT - DDT (FDT model is in other code)
